...
import com.example.gatewayrestaurant.databinding.FragmentIndianStarterBinding
...
class IndianStarterFragment : BaseFragment() {

    companion object {

        @SuppressLint("StaticFieldLeak")
        private var mBinding: FragmentIndianStarterBinding? = null
        fun disableThem() {
            if (mBinding != null) {
    ...
    var indianStarterAdapter: MenuAdapter? = null
    ...
    override fun onCreateView(
        ...
        mBinding = DataBindingUtil.inflate(
            inflater,
            R.layout.fragment_indian_starter,
            container,
            false
        )
        enableThem()
        ...
        val options = FirebaseRecyclerOptions.Builder<MenuModel>()
            .setQuery(
                FirebaseDatabase.getInstance().reference.child("indianStarter").orderByChild("name"),
                MenuModel::class.java
            )
            .build()

        indianStarterAdapter =
            MenuAdapter(
                options,
                mActivity,
                callBack
            )
        mBinding!!.rvDishList.adapter = indianStarterAdapter
        return mBinding!!.root
    }

    override fun onStart() {
        super.onStart()
        indianStarterAdapter!!.startListening()
    }

    override fun onStop() {
        super.onStop()
        indianStarterAdapter!!.stopListening()
    }
    ...
    private fun searchDish(s: String) {
        val options1 = FirebaseRecyclerOptions.Builder<MenuModel>()
            .setQuery(
                FirebaseDatabase.getInstance().reference.child("indianStarter")
